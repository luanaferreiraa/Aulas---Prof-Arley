...
async function buscarCidades() {
  ...
  try {
    ...
    elementoMensagem.textContent =
      `${dados.results.length} cidade(s) encontrada(s).`;
...
async function buscarPrevisao(latitude, longitude, cidade, estado) {
  ...
  try {
    ...
    let dias = dados.daily.time
      .map((data, indice) => {
        let condicao =
          descricaoClima(dados.daily.weather_code[indice]);
        ...
        let indiceUv =
          Number(dados.daily.uv_index_max[indice] ?? 0).toFixed(1);
        ...
        return `
          <article class="dia">
            ...
            <div class="temperaturas">
              <span>
                <strong>${minima} °C</strong>
                Mínima
              </span>

              <span>
                <strong>${maxima} °C</strong>
                Máxima
              </span>
            </div>
